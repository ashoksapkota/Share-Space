...
export async function getActivity(userId: string) {
  try {
    ...
    const [userEchoes, user] = await Promise.all([
      Echo.find({ author: userId }),
      User.findOne({ _id: userId }),
    ]);

    const childEchoIds = userEchoes.flatMap(
      (userEcho) => userEcho.children
    );
    const reactions = userEchoes.flatMap((userEcho) => userEcho.reactions);
    ...
    const reactionsData = reactions.map((reaction, index) => {
      ...
      return {
        ...
        parentId: userEchoes[0]._id.toString(),
